...
type Horario = { day: number; on: boolean; inicio: string; fim: string };
...
export default function TurmasPage() {
  ...
  const listaFiltrada = lista.filter((t) => {
    ...
    const alvo = [t.nome, t.nivel || "", t.curso || "", t.professor?.nome || ""].join(" ");
  ...
  return (
    <main style={{ padding: 24, maxWidth: 960, margin: "0 auto" }}>
      ...
      <input
        placeholder="Buscar por nome/nivel/curso/professor"
        value={filtro}
      ...
      <form onSubmit={criar} style={{ display: "grid", gap: 10, margin: "12px 0" }}>
        ...
        <div style={{ display: "flex", gap: 18, alignItems: "center" }}>
          <label>
            <input
              type="checkbox"
              checked={form.particular}
              onChange={(e) => setForm({ ...form, particular: e.target.checked })}
            />{" "}
            Turma Particular
          </label>
          <label>
            <input
              type="checkbox"
              checked={form.passe_livre}
              onChange={(e) => setForm({ ...form, passe_livre: e.target.checked })}
            />{" "}
            Passe Livre
          </label>
          <label>
      ...
      <ul style={{ marginTop: 16 }}>
        {listaFiltrada.map((t) => (
          <li
            key={t.id}
            style={{ marginBottom: 12, borderBottom: "1px solid #333", paddingBottom: 8 }}
          >
            <b>#{t.id}</b> — {t.nome}
            {t.nivel ? ` • ${t.nivel}` : ""} {t.curso ? ` • ${t.curso}` : ""}
            {t.capacidade ? ` • cap ${t.capacidade}` : ""}
            {t.professor ? ` • prof. ${t.professor.nome}` : ""}
            {t.particular ? " • particular" : ""}
            {t.passe_livre ? " • passe livre" : ""}
            {t.online ? " • online" : ""}
              ...
              {t.horarios?.length ? (
                <div style={{ marginTop: 4 }}>
                  {t.horarios.map((h) => {
                    ...
                    return (
                      <span key={`${t.id}-${h.day_of_week}-${h.inicio}`} style={{ marginRight: 10 }}>
                        {nomeDia}: {h.inicio}–{h.fim}
                      </span>
                    );
                  })}
                </div>
