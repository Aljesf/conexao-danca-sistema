-- Dashboard Escola - refinamento analitico do detalhamento por vinculo
-- Esta view apoia o dashboard da Escola com drill-down operacional por vinculo
-- ativo de turma/matricula. No escopo institucional, o mesmo aluno pode aparecer
-- mais de uma vez quando possui mais de uma turma/modalidade ativa. Isso e
-- comportamento intencional do produto e nao substitui relatorios completos.
--
-- Regra de valor mensal estimado por vinculo:
-- 1) Prioriza public.matricula_execucao_valores por matricula/turma ativa.
-- 2) Valores identificados como familia entram na parcela sustentada pela familia.
-- 3) Valores identificados como bolsa/movimento entram na parcela institucional.
-- 4) Valores genericos de execucao sao alocados pela forma operacional da matricula:
--    CREDITO_BOLSA -> institucional; demais casos -> familia.
-- 5) Se nao houver execucao para o vinculo, usa fallback seguro em
--    public.matriculas.total_mensalidade_centavos.

begin;

drop view if exists public.vw_escola_dashboard_alunos_detalhe;

create view public.vw_escola_dashboard_alunos_detalhe as
with turmas_base as (
  select
    t.turma_id,
...
vinculos_classificados as (
  select
    ...
    vc.matricula_id,
    true as aluno_ativo,
    coalesce(n.nome, e.nivel_execucao, tb.nivel, 'Nao informado') as nivel_ou_serie,
    case
    ...
    case
      when vc.matricula_id is null then 'ATIVO'
      when (
        (
          case
            when e.valor_total_execucao_centavos is not null then
...
select
  ...
  vc.concessao_tipo,
  (vc.valor_familia_centavos + vc.valor_institucional_centavos)::bigint
    as valor_mensal_estimado_centavos,
...
'View canonica de apoio ao dashboard da Escola para drill-down operacional por vinculo ativo de turma. No escopo institucional, o mesmo aluno pode aparecer repetido em turmas/modalidades diferentes. Nao substitui relatorios completos.';
