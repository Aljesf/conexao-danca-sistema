...
-- 1) Derruba dependencias primeiro
DROP VIEW IF EXISTS public.vw_financeiro_devedores_atrasados;
DROP VIEW IF EXISTS public.vw_financeiro_contas_receber_flat;

-- 2) Recria a view base DO ZERO (sem OR REPLACE) e com lista explicita de colunas
-- IMPORTANTE:
-- Ajuste se o seu schema NAO tiver:
--   public.cobrancas (id, pessoa_id, vencimento, status, origem_tipo, origem_id, created_at, valor_centavos)
--   public.recebimentos (cobranca_id, valor_centavos)
-- Se os nomes forem diferentes, adapte conforme o schema real.
...
SELECT
  b.cobranca_id,
  b.pessoa_id,
  b.vencimento AS data_vencimento,
  b.status_cobranca,
  b.origem_tipo,
  b.origem_id,
  b.created_at,
  b.valor_total_centavos,
  ...
  -- Situacao SaaS (a "verdade" da cobranca)
  CASE
...
'View SaaS canonica: separa situacao_saas (QUITADA/EM_ABERTO/VENCIDA) do status interno e garante atraso coerente com saldo_aberto.';

-- 3) Recria a view agregada (devedores atrasados)
...
'Top devedores atrasados (SaaS): somente titulos VENCIDA com saldo aberto.';
