-- Dashboard Escola - refinamento operacional de modalidade e financeiro
-- Esta migration ajusta a leitura do dashboard da Escola para refletir que
-- alunos ativos representam a ocupacao real de cada turma, enquanto vagas
-- disponiveis derivam diretamente da capacidade cadastrada.
--
-- Receita mensal estimada total = receita pagante + absorcao institucional.
-- Essa leitura e operacional e institucional; nao substitui o financeiro
-- oficial, conciliacao, fechamento contabil ou relatorios analiticos formais.
--
-- Fontes canonicas utilizadas:
-- - public.turmas
-- - public.turma_aluno
-- - public.matriculas
-- - public.pessoas
--
-- Nao usa tabelas legadas de alunos ou alunos_turmas.
...
with turmas_base as (
  select
    ...
    regexp_replace(
      translate(
        lower(coalesce(t.curso, 'sem-curso')),
...
vinculos_classificados as (
  select
    ...
    vp.matricula_id,
    vp.nivel_composicao,
    vp.valor_familia_centavos,
    vp.valor_institucional_centavos,
    (vp.valor_familia_centavos + vp.valor_institucional_centavos)::bigint
      as valor_total_estimado_centavos,
    case
      when vp.matricula_id is null then 'OUTRO'
      when (
        vp.valor_institucional_centavos > 0
        or coalesce(ba.concessoes_ativas, 0) > 0
      ) then
        case
          when (vp.valor_institucional_centavos > 0 and vp.valor_familia_centavos > 0)
      ...
      when vp.matricula_id is not null then 'PAGANTE'
      else 'OUTRO'
    end as vinculo_institucional
  from vinculos_precificados vp
  left join bolsas_ativas_por_vinculo ba
    on ba.turma_aluno_id = vp.turma_aluno_id
),
agregados_por_turma as (
  select
    ...
    count(*) filter (where vc.vinculo_institucional = 'CONCESSAO_INTEGRAL')::int
      as concessao_integral_total,
    count(*) filter (where vc.vinculo_institucional = 'CONCESSAO_PARCIAL')::int
      as concessao_parcial_total,
    count(*) filter (where vc.vinculo_institucional = 'OUTRO')::int as outros_vinculos_total,
    coalesce(sum(vc.valor_total_estimado_centavos), 0)::bigint
      as receita_mensal_estimada_centavos,
    coalesce(sum(vc.valor_familia_centavos), 0)::bigint
      as receita_pagante_estimada_centavos,
    coalesce(sum(vc.valor_institucional_centavos), 0)::bigint
      as receita_concessao_absorvida_centavos
  from turmas_base tb
...
select
  ...
  case
    when tb.capacidade is not null and tb.capacidade >= 0
      then tb.capacidade - coalesce(ag.alunos_ativos_total, 0)
    else null
  ...
  tb.curso_slug_ou_chave_filtro,
  coalesce(ag.receita_mensal_estimada_centavos, 0)::bigint as receita_mensal_estimada_centavos,
  coalesce(ag.receita_pagante_estimada_centavos, 0)::bigint as receita_pagante_estimada_centavos,
  coalesce(ag.receita_concessao_absorvida_centavos, 0)::bigint
    as receita_concessao_absorvida_centavos
from turmas_base tb
...
create or replace view public.vw_escola_dashboard_modalidade_resumo as
select
  coalesce(nullif(btrim(t.curso), ''), 'Sem curso') as curso,
  count(*)::bigint as total_turmas,
  coalesce(sum(t.alunos_ativos_total), 0)::bigint as alunos_ativos_total,
  coalesce(sum(t.pagantes_total), 0)::bigint as pagantes_total,
  coalesce(sum(t.concessao_total), 0)::bigint as concessao_total,
  coalesce(sum(t.concessao_integral_total), 0)::bigint as concessao_integral_total,
  coalesce(sum(t.concessao_parcial_total), 0)::bigint as concessao_parcial_total,
  coalesce(sum(t.receita_mensal_estimada_centavos), 0)::bigint
    as receita_mensal_estimada_centavos,
  coalesce(sum(t.receita_pagante_estimada_centavos), 0)::bigint
    as receita_pagante_estimada_centavos,
  coalesce(sum(t.receita_concessao_absorvida_centavos), 0)::bigint
    as receita_concessao_absorvida_centavos
from public.vw_escola_dashboard_turmas_composicao t
group by
  coalesce(nullif(btrim(t.curso), ''), 'Sem curso');

create or replace view public.vw_escola_dashboard_resumo_institucional as
select
  coalesce(sum(t.pagantes_total), 0)::bigint as pagantes_total,
  coalesce(sum(t.concessao_total), 0)::bigint as concessao_total,
  coalesce(sum(t.concessao_integral_total), 0)::bigint as concessao_integral_total,
...
'View canonica do dashboard da Escola para composicao operacional por turma. Alunos ativos representam a ocupacao real; vagas disponiveis derivam da capacidade cadastrada; receita total estimada = receita pagante + absorcao institucional. Nao substitui relatorios financeiros oficiais.';

comment on view public.vw_escola_dashboard_modalidade_resumo is
'Resumo operacional por curso/modalidade do dashboard da Escola, agregando turmas, alunos ativos, composicao institucional e leitura financeira estimada. Nao substitui relatorios financeiros oficiais.';

comment on view public.vw_escola_dashboard_resumo_institucional is
'Resumo institucional do dashboard da Escola com pagantes, concessoes e financeiro operacional estimado. Receita total estimada = receita pagante + absorcao institucional. Nao substitui o financeiro oficial.';
