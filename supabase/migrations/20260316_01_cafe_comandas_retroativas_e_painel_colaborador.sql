...
-- Cafe: comandas retroativas, baixa parcial e envio para conta interna do colaborador.
-- Esta migration reaproveita public.cafe_vendas/public.cafe_venda_itens.
...
set
  ...
  tipo_quitacao = case
    when upper(coalesce(forma_pagamento, '')) like '%CARTAO_CONEXAO_COLABORADOR%' then 'CONTA_INTERNA_COLABORADOR'
    when upper(coalesce(status_pagamento, '')) = 'PAGO' then 'IMEDIATA'
    else 'PARCIAL'
  end,
  data_competencia = case
    when upper(coalesce(forma_pagamento, '')) like '%CARTAO_CONEXAO_COLABORADOR%'
...
begin
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'cafe_vendas_pago_sem_aberto_chk'
      and conrelid = 'public.cafe_vendas'::regclass
  ) then
    alter table public.cafe_vendas
      add constraint cafe_vendas_pago_sem_aberto_chk
      check (
        status_pagamento <> 'PAGO'
        or valor_em_aberto_centavos = 0
      );
  end if;
end $$;
...
comment on table public.cafe_vendas is
'Comanda/venda operacional do Ballet Cafe, com suporte a venda retroativa, baixa parcial e envio para conta interna do colaborador por competencia.';

comment on column public.cafe_vendas.data_operacao is
'Data operacional da comanda. Permite lancamento retroativo sem alterar a trilha de created_at.';

comment on column public.cafe_vendas.tipo_quitacao is
'Liquidação da comanda: IMEDIATA, PARCIAL ou CONTA_INTERNA_COLABORADOR.';

comment on column public.cafe_vendas.data_competencia is
'Competencia YYYY-MM usada quando o saldo segue para a conta interna do colaborador.';

comment on column public.cafe_vendas.cobranca_id is
'Cobranca canonica por competencia vinculada a comandas do cafe quando a origem segue para conta interna do colaborador.';

comment on column public.cafe_venda_itens.descricao_snapshot is
'Snapshot textual do produto na data da venda, preservando historico mesmo com mudancas futuras no catalogo.';
