-- Dashboard Escola - refinamento de KPIs institucionais e receita estimada
-- Esta migration reforca a base operacional do dashboard SaaS da Escola.
-- A receita aqui e uma estimativa operacional/institucional por turma e por escola,
-- calculada a partir da execucao canonica de matricula por turma. Nao substitui
-- relatorios financeiros oficiais, contabilizacao ou conciliacao financeira.
--
-- Regra adotada para estimativa mensal:
-- 1) Prioriza public.matricula_execucao_valores por matricula/turma ativa, pois esta e
--    a fonte canonica mais granular da composicao mensal por turma.
-- 2) Valores identificados como MANUAL|FAMILIA entram como parcela sustentada pela familia.
-- 3) Valores identificados como MANUAL|BOLSA* / MANUAL|MOVIMENTO* entram como absorcao institucional.
-- 4) Valores genericos de execucao (ex.: MANUAL sem sufixo) sao alocados pela forma operacional
--    da matricula: CREDITO_BOLSA -> institucional; demais casos -> familia.
-- 5) Se uma matricula/turma ativa nao tiver execucao registrada, o fallback seguro e
--    public.matriculas.total_mensalidade_centavos, respeitando o metodo_liquidacao.
...
select
  ...
  coalesce(sum(t.receita_mensal_estimada_centavos), 0)::bigint as receita_mensal_estimada_centavos
from public.vw_escola_dashboard_turmas_composicao t;

comment on view public.vw_escola_dashboard_turmas_composicao is
'View canonica do dashboard SaaS da Escola para composicao operacional, institucional e receita mensal estimada por turma. Usa turma_aluno + matriculas + matricula_execucao_valores e nao substitui relatorios analiticos ou financeiros oficiais.';

comment on view public.vw_escola_dashboard_resumo_institucional is
'Resumo institucional do dashboard SaaS da Escola com totais operacionais de pagantes, concessoes e receita mensal estimada. Nao substitui relatorio financeiro oficial.';
