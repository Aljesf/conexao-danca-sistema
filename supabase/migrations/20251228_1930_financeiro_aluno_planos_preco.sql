...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'financeiro_aluno_planos_preco_politica_fk') THEN
    ALTER TABLE public.financeiro_aluno_planos_preco
      DROP CONSTRAINT financeiro_aluno_planos_preco_politica_fk;
  END IF;

  ALTER TABLE public.financeiro_aluno_planos_preco
    ADD CONSTRAINT financeiro_aluno_planos_preco_politica_fk
    FOREIGN KEY (politica_id) REFERENCES public.financeiro_politicas_preco(politica_preco_id)
    ON DELETE RESTRICT;
END $$;
