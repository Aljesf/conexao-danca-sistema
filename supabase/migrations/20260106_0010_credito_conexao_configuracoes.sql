-- =========================================
-- Credito Conexao - Configuracoes gerais do cartao
-- - ciclo: dia_fechamento / dia_vencimento
-- - politica de atraso: multa / juros ao dia / tolerancia
-- - por tipo_conta: ALUNO | COLABORADOR
-- =========================================
...
create index if not exists idx_cc_config_ativo
  on public.credito_conexao_configuracoes(ativo);

-- seed padrao (somente se nao existir)
...
-- trigger updated_at especifico (sem sobrescrever global)
do $$
begin
  if not exists (
    select 1
  ...
  if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_cc_config_updated_at'
  ) then
    execute $trg$
