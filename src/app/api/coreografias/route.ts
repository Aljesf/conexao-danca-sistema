...
import {
  criarCoreografiaMestreEvento,
  listarCoreografiasMestresEvento,
} from "@/lib/eventos/service";
import { validateCoreografiaMestrePayload } from "@/lib/eventos/validators";
...
export async function GET() {
  try {
    ...
    const data = await listarCoreografiasMestresEvento(db);

    return NextResponse.json({ ok: true, data }, { status: 200 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "erro interno";
    console.error("COREOGRAFIAS GET ERROR:", error);
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateCoreografiaMestrePayload(body);
    const data = await criarCoreografiaMestreEvento(db, payload);

    return NextResponse.json({ ok: true, data }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "erro interno";
    console.error("COREOGRAFIAS POST ERROR:", error);

    return NextResponse.json(
      { ok: false, error: "falha ao criar coreografia", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
