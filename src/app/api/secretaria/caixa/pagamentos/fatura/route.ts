...
export async function POST(request: NextRequest) {
  ...
  const faturaId = parsePositiveInt(body?.fatura_id);
  ...
  if (!faturaId) {
    return NextResponse.json({ ok: false, error: "fatura_id_obrigatorio" }, { status: 400 });
  }
  ...
  try {
    ...
    console.error("[SECRETARIA][CAIXA][PAGAMENTO_FATURA][ERRO]", { faturaId, message });
    return NextResponse.json(
      {
        ok: false,
        error: "falha_pagamento_fatura_secretaria",
