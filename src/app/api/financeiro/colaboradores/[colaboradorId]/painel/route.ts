...
import { getColaboradorFinanceiroPainel } from "@/lib/financeiro/colaboradoresFinanceiro";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ colaboradorId: string }> }) {
  ...
  const data = await getColaboradorFinanceiroPainel(getSupabaseAdmin(), colaboradorId);
  if (!data) {
