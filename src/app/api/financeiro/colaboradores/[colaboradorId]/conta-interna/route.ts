...
import { getColaboradorContaInterna } from "@/lib/financeiro/colaboradoresFinanceiro";
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ colaboradorId: string }> }) {
  ...
  const data = await getColaboradorContaInterna(getSupabaseAdmin(), colaboradorId);
  if (!data) {
