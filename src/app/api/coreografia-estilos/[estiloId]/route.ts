...
import {
  arquivarCoreografiaEstiloEvento,
  atualizarCoreografiaEstiloEvento,
  buscarCoreografiaEstiloEvento,
} from "@/lib/eventos/service";
import { validateCoreografiaEstiloUpdatePayload } from "@/lib/eventos/validators";
...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const { estiloId } = await params;
    const data = await buscarCoreografiaEstiloEvento(supabase, estiloId);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao carregar estilo", details: message },
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const payload = validateCoreografiaEstiloUpdatePayload({ ...body, estiloId });
    const data = await atualizarCoreografiaEstiloEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao atualizar estilo", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
...
export async function DELETE(_request: Request, { params }: Params) {
  try {
    const { supabase } = await requireAuthenticatedUser();
    const { estiloId } = await params;
    const data = await arquivarCoreografiaEstiloEvento(supabase, estiloId);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao arquivar estilo", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
