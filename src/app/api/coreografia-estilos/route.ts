...
import {
  criarCoreografiaEstiloEvento,
  listarCoreografiaEstilosEvento,
} from "@/lib/eventos/service";
import { validateCoreografiaEstiloPayload } from "@/lib/eventos/validators";
...
export async function GET() {
  try {
    ...
    const data = await listarCoreografiaEstilosEvento(supabase);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao listar estilos", details: message },
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateCoreografiaEstiloPayload(body);
    const data = await criarCoreografiaEstiloEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar estilo", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
