...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (conjErr || !conjunto) {
    return NextResponse.json({ ok: false, message: "Conjunto não encontrado." }, { status: 404 });
  }
  ...
  const grupoSet = new Map<number, { obrigatorio: boolean }>();
  for (const g of (grupos ?? []) as unknown as Array<Record<string, unknown>>) {
    const gid = Number(g.id);
    grupoSet.set(gid, { obrigatorio: Boolean(g.obrigatorio) });
  }
  ...
  if (faltandoObrig.length > 0) {
    return NextResponse.json(
      { ok: false, message: `Grupos obrigatórios não selecionados: ${faltandoObrig.join(", ")}` },
      { status: 400 }
    );
  }
  ...
  if (grupoIdsIncluidos.length === 0) {
    return NextResponse.json({ ok: false, message: "Nenhum grupo selecionado para emissão." }, { status: 400 });
  }
  ...
  for (const item of selecionados.filter((s) => s.incluir)) {
    if (!grupoSet.has(item.grupo_id)) {
      return NextResponse.json({ ok: false, message: `Grupo inválido para este conjunto: ${item.grupo_id}` }, { status: 400 });
    }

    const key = `${item.grupo_id}:${item.documento_modelo_id}`;
    if (!allowed.has(key)) {
      return NextResponse.json(
        { ok: false, message: `Modelo ${item.documento_modelo_id} não está vinculado ao grupo ${item.grupo_id}.` },
        { status: 400 }
      );
    }
    ...
    if (modErr || !modelo) {
      return NextResponse.json({ ok: false, message: `Modelo não encontrado: ${item.documento_modelo_id}` }, { status: 404 });
    }
  ...
  return NextResponse.json({ ok: true, data: { matricula_id: matriculaId, documento_conjunto_id: conjuntoId, emitidos } }, { status: 201 });
}
