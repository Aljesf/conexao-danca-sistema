...
import { criarSessaoEvento } from "@/lib/eventos/service";
import { validateSessaoPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateSessaoPayload(body);
    const data = await criarSessaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar sessão", details: message },
      { status },
    );
  }
}
