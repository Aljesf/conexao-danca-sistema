...
import { criarAtividadeSessaoEvento } from "@/lib/eventos/service";
import { validateSessaoAtividadePayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateSessaoAtividadePayload(body);
    const data = await criarAtividadeSessaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar atividade da sessão", details: message },
      { status },
    );
  }
}
