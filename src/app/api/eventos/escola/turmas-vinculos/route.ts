...
import { criarTurmaVinculoEvento } from "@/lib/eventos/service";
import { validateTurmaVinculoPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateTurmaVinculoPayload(body);
    const data = await criarTurmaVinculoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar vínculo de turma", details: message },
      { status },
    );
  }
}
