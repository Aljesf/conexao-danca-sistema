...
import { adicionarParticipanteCoreografiaEvento } from "@/lib/eventos/service";
import { validateCoreografiaParticipantePayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateCoreografiaParticipantePayload(body);
    const data = await adicionarParticipanteCoreografiaEvento(supabase, payload);
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "falha ao adicionar participante da coreografia",
        details: message,
      },
      { status },
    );
