...
import { criarModalidadeEvento } from "@/lib/eventos/service";
import { validateModalidadePayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateModalidadePayload(body);
    const data = await criarModalidadeEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar modalidade", details: message },
      { status },
    );
  }
}
