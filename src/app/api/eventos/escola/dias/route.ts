...
import { criarDiaEvento } from "@/lib/eventos/service";
import { validateDiaPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateDiaPayload(body);
    const data = await criarDiaEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar dia do evento", details: message },
      { status },
    );
  }
}
