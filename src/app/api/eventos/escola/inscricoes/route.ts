...
import { criarInscricaoEvento } from "@/lib/eventos/service";
import { validateInscricaoPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateInscricaoPayload(body);
    const data = await criarInscricaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar inscrição", details: message },
      { status },
    );
  }
}
