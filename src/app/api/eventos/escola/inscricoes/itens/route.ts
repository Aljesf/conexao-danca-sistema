...
import { criarItemInscricaoEvento } from "@/lib/eventos/service";
import { validateInscricaoItemPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateInscricaoItemPayload(body);
    const data = await criarItemInscricaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar item da inscrição", details: message },
      { status },
    );
  }
}
