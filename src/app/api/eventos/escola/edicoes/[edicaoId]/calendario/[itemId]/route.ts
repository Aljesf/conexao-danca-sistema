...
import {
  arquivarItemCalendarioEdicaoEvento,
  atualizarItemCalendarioEdicaoEvento,
} from "@/lib/eventos/service";
import { validateEdicaoCalendarioUpdatePayload } from "@/lib/eventos/validators";
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const { edicaoId, itemId } = await params;
    const body = (await request.json()) as Record<string, unknown>;
    const payload = validateEdicaoCalendarioUpdatePayload({
      ...body,
      edicaoId,
      itemId,
    });
    const data = await atualizarItemCalendarioEdicaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao atualizar item do calendÃ¡rio", details: message },
...
export async function DELETE(_request: Request, { params }: Params) {
  try {
    const { supabase } = await requireAuthenticatedUser();
    const { edicaoId, itemId } = await params;
    const data = await arquivarItemCalendarioEdicaoEvento(supabase, edicaoId, itemId);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao arquivar item do calendÃ¡rio", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
