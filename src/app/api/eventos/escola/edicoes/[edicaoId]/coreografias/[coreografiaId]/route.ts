...
import {
  arquivarCoreografiaEvento,
  atualizarVinculoCoreografiaEdicaoEvento,
} from "@/lib/eventos/service";
import { validateEventoEdicaoCoreografiaVinculoUpdatePayload } from "@/lib/eventos/validators";
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const { edicaoId, coreografiaId } = await params;
    const body = (await request.json()) as Record<string, unknown>;
    const payload = validateEventoEdicaoCoreografiaVinculoUpdatePayload({
      ...body,
      edicaoId,
      vinculoId: coreografiaId,
    });
    const data = await atualizarVinculoCoreografiaEdicaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao atualizar vinculo da coreografia", details: message },
...
export async function DELETE(_request: Request, { params }: Params) {
  try {
    const { supabase } = await requireAuthenticatedUser();
    const { edicaoId, coreografiaId } = await params;
    const data = await arquivarCoreografiaEvento(supabase, edicaoId, coreografiaId);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao arquivar vinculo da coreografia", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
