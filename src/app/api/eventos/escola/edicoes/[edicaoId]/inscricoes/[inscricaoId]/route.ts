...
import {
  arquivarInscricaoEdicaoEvento,
  atualizarInscricaoEdicaoEvento,
  buscarInscricaoEdicaoEvento,
} from "@/lib/eventos/service";
import { validateEdicaoInscricaoUpdatePayload } from "@/lib/eventos/validators";
...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao buscar inscricao", details: message },
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const payload = validateEdicaoInscricaoUpdatePayload({
      ...body,
      edicaoId,
      inscricaoId,
    });
    const data = await atualizarInscricaoEdicaoEvento(db, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao atualizar inscricao", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
...
export async function DELETE(_request: Request, { params }: Params) {
  try {
    const { db } = await requireAuthenticatedUser();
    const { edicaoId, inscricaoId } = await params;
    const data = await arquivarInscricaoEdicaoEvento(db, edicaoId, inscricaoId);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao cancelar inscricao", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
