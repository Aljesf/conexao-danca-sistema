...
import { cancelarItemInscricaoEdicaoEvento } from "@/lib/eventos/service";
import { validateEdicaoInscricaoCancelarItemPayload } from "@/lib/eventos/validators";
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const { edicaoId, inscricaoId, itemId } = await params;
    const body = (await request.json().catch(() => ({}))) as Record<string, unknown>;
    const payload = validateEdicaoInscricaoCancelarItemPayload({
      ...body,
      edicaoId,
      inscricaoId,
      itemId,
    });
    const data = await cancelarItemInscricaoEdicaoEvento(db, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao cancelar item da inscricao", details: message },
