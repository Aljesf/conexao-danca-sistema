...
import { reprocessarFinanceiroInscricaoEdicaoEvento } from "@/lib/eventos/service";
...
export async function POST(_request: Request, { params }: Params) {
  try {
    ...
    const data = await reprocessarFinanceiroInscricaoEdicaoEvento(
      db,
      { edicaoId, inscricaoId },
      { userId: user.id },
    );

    return NextResponse.json({ ok: true, success: true, data }, { status: 200 });
  } catch (error) {
    console.error("EVENTOS EDICAO INSCRICOES REPROCESSAR FINANCEIRO POST ERROR:", error);
    const errorResponse = getErrorResponse(
      error,
      "falha ao reprocessar financeiro da inscricao",
    );
