...
import { criarEdicaoEvento } from "@/lib/eventos/service";
import { validateEdicaoPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateEdicaoPayload(body);
    const data = await criarEdicaoEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar edição", details: message },
      { status },
    );
  }
}
