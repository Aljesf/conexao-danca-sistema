...
import {
  criarParticipanteExternoEvento,
  listarParticipantesExternosEvento,
} from "@/lib/eventos/service";
import { validateParticipanteExternoPayload } from "@/lib/eventos/validators";
...
export async function GET(request: Request) {
  try {
    ...
    const data = await listarParticipantesExternosEvento(db, q);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao listar participantes externos", details: message },
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateParticipanteExternoPayload(body);
    const data = await criarParticipanteExternoEvento(db, payload, {
      userId: user.id,
    });
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao criar participante externo", details: message },
      { status: getStatusFromMessage(message) },
    );
  }
}
