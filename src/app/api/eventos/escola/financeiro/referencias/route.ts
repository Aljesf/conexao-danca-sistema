...
import { criarReferenciaFinanceiraEvento } from "@/lib/eventos/service";
import { validateFinanceiroReferenciaPayload } from "@/lib/eventos/validators";
...
export async function POST(request: Request) {
  try {
    ...
    const payload = validateFinanceiroReferenciaPayload(body);
    const data = await criarReferenciaFinanceiraEvento(supabase, payload);
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "falha ao criar referência financeira do evento",
        details: message,
      },
      { status },
    );
