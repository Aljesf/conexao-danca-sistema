...
import { atualizarEventoEscolaEvento } from "@/lib/eventos/service";
import { validateEventoUpdatePayload } from "@/lib/eventos/validators";
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const payload = validateEventoUpdatePayload({ ...body, eventoId });
    const data = await atualizarEventoEscolaEvento(supabase, payload);
    ...
    return NextResponse.json(
      { ok: false, error: "falha ao atualizar evento", details: message },
