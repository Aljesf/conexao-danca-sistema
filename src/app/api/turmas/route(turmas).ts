...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("turmas")
    .select(`
      id, nome, nivel, modalidade, capacidade, ativo, created_at, user_email,
      particular, passe_livre, online, professor_id,
      professor:pessoa ( pessoa_id:id, nome_completo:nome, email ),
