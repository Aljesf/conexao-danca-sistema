...
export default function PlanoContasPage() {
  const [contas, setContas] = useState<ContaContabil[]>(seedPlanoContas);
  ...
  function salvarNovaConta(e: React.FormEvent) {
    e.preventDefault();
    if (!form.codigo.trim() || !form.nome.trim()) return;
    const novo: ContaContabil = {
      id: contas.length ? Math.max(...contas.map((c) => c.id)) + 1 : 1,
      codigo: form.codigo,
      nome: form.nome,
      tipo: form.tipo,
      parentId: form.parentId || null,
    };
    setContas((prev) => [...prev, novo]);
    setShowModal(false);
    resetForm();
  }

  const paisPorTipo = useMemo(
