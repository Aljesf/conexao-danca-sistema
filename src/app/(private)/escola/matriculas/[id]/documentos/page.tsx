...
type Grupo = { id: number; conjunto_id: number; nome: string; codigo: string; obrigatorio: boolean; ordem: number };
type Modelo = {
  id: number;
  titulo: string;
  versao: string;
...
export default function MatriculaDocumentosPage(props: { params: { id: string } }) {
  const matriculaId = Number(props.params.id);
  ...
  const helpItems = useMemo(
    () => [
      "Fluxo manual: escolha o Conjunto → selecione os Grupos → escolha um Modelo por Grupo → emitir.",
      "Grupos obrigatórios ficam sempre incluídos.",
      "Modelos disponíveis por grupo dependem do vínculo Grupo ↔ Modelos no Admin.",
    ],
    []
  );
  ...
  const carregarBase = useCallback(async () => {
    ...
    try {
      ...
      if (!resM.ok || !jsonM.ok) throw new Error(jsonM.message ?? "Falha ao carregar matrícula.");
  ...
  const carregarGruposEModelos = useCallback(async (conjId: number) => {
    ...
    for (const g of gs) {
      inc[g.id] = g.obrigatorio ? true : false;
      const opts = map[g.id] ?? [];
  ...
  const salvarConjuntoNaMatricula = useCallback(async () => {
    ...
    try {
      ...
      setOkMsg("Conjunto salvo na matrícula.");
    } catch (e) {
  ...
  async function emitir() {
    ...
    const itens = grupos.map((g) => ({
      grupo_id: g.id,
      incluir: incluirGrupo[g.id] ?? g.obrigatorio,
    ...
    if (faltando.length > 0) {
      setErro("Selecione um modelo para todos os grupos incluídos.");
      return;
    }
  ...
  return (
    <SystemPage>
      <SystemContextCard
        title="Matrícula — Documentos"
        subtitle="Seleção manual: Conjunto → Grupos → Modelo por Grupo → Emissão."
      >
      ...
      <SystemSectionCard title="1) Selecionar conjunto" description="Escolha manualmente qual conjunto de documentos se aplica a esta matrícula.">
        {loading ? (
      ...
      <SystemSectionCard title="2) Selecionar grupos e modelos" description="Marque os grupos que deseja emitir e selecione um modelo por grupo.">
        {conjuntoId ? (
          grupos.length === 0 ? (
            <div className="text-sm text-slate-600">Este conjunto não possui grupos cadastrados.</div>
          ) : (
            <div className="grid gap-3">
              {grupos.map((g) => {
                const opts = modelosPorGrupo[g.id] ?? [];
                const isObrig = g.obrigatorio;
                  ...
                  <div key={g.id} className="border border-slate-200 rounded-xl p-4 bg-white">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="text-xs text-slate-600 mt-1">
                          Código: {g.codigo} • Obrigatório: {isObrig ? "Sim" : "Não"}
                        </div>
                      </div>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="mt-2 text-xs text-slate-600">
                        Se a lista estiver vazia, vincule modelos no Admin: Conjuntos → Grupos → “Vincular modelos”.
                      </div>
                    </div>
                  </div>
                );
