...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const { data: emitidos, error } = await supabase
    ...
    .select("id, matricula_id, documento_modelo_id, contrato_modelo_id, status, created_at")
    ...
    <div className="p-6 max-w-5xl space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Link
            href={`/admin/config/documentos/modelos?emitirParaMatriculaId=${matriculaId}`}
      ...
      <div className="rounded-lg border p-4 space-y-3">
        ...
        {lista.length === 0 ? (
          ...
          <div className="grid gap-3">
            {lista.map((doc) => {
              const modeloId = doc.documento_modelo_id ?? doc.contrato_modelo_id ?? null;
              return (
