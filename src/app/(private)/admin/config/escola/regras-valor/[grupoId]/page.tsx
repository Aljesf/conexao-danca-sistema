...
export default function AdminConfigEscolaRegrasValorGrupoPage(
  ...
  const [grupo, setGrupo] = React.useState<TierGrupo | null>(null);
  ...
  const carregar = React.useCallback(async () => {
    ...
    try {
      const [resGrupos, resTiers, resPoliticas, resTabelas] = await Promise.all([
        fetch("/api/admin/escola/regras-valor/grupos", { method: "GET" }),
        fetch(`/api/admin/escola/regras-valor/tiers?grupo_id=${grupoId}`, { method: "GET" }),
      ...
      const g = (jsonGrupos.grupos ?? []).find((x) => Number(x.tier_grupo_id) === grupoId) ?? null;
      setGrupo(g);
  ...
  async function criarTier() {
    ...
    const politicaNum = parsePositiveInt(politicaId);
  ...
  const groupedTiers = React.useMemo(() => {
    const politicaMap = new Map(politicas.map((p) => [p.id, p]));
    const grupos = new Map<string, { politicaId: number | null; label: string; tiers: Tier[] }>();
      ...
      <SectionCard title="Dados do grupo">
        {loading ? (
          ...
          <div className="space-y-1 text-sm text-slate-700">
            <div>
              <span className="font-medium text-slate-900">Nome:</span> {grupo.nome || "(sem nome)"}
            </div>
            <div>
              <span className="font-medium text-slate-900">Descricao:</span> {grupo.descricao || "-"}
            </div>
            <div>
              <span className="font-medium text-slate-900">Ativo:</span> {grupo.ativo ? "Sim" : "Nao"}
            </div>
          </div>
        ) : (
          <div className="text-sm text-muted-foreground">Grupo nao encontrado.</div>
        )}
      </SectionCard>
                    ...
                    <tbody>
                      {grupo.tiers.map((t) => (
                        <tr key={String(t.tier_id)} className="border-b">
                          ...
                          <td className="py-2 pr-3">
                            {formatAjusteLabel(t.ajuste_tipo, t.ajuste_valor_centavos)}
                          </td>
