...
type FaturaRow = { id: number; conta_conexao_id: number | null; periodo_referencia: string | null; data_fechamento: string | null; data_vencimento: string | null; status: string | null; valor_total_centavos: number | null; cobranca_id: number | null; folha_pagamento_id?: number | null; neofin_invoice_id?: string | null; updated_at?: string | null };
...
type ItemOperacional = { descricao: string; quantidade: number | null; valor_unitario_centavos: number | null; valor_total_centavos: number; detalhe: string | null; };
type LancamentoRow = { id: number; origem_sistema: string | null; descricao: string | null; descricao_operacional?: string | null; valor_centavos: number | null; competencia: string | null; referencia_item: string | null; status: string | null; composicao_json: Record<string, unknown> | null; itens_operacionais?: ItemOperacional[]; aluno_nome?: string | null; responsavel_financeiro_nome?: string | null; cobranca_fatura_id?: number | null };
type ContextoTitular = { tipo: "COLABORADOR" | "ALUNO" | "OUTRO"; conta_interna_id?: number | null; colaborador_id: number | null; competencia: string | null; folha_pagamento_colaborador_id: number | null; status_importacao_folha: string | null; titular_label: string; };
type StatusOrigem = "VALIDA" | "CANCELADA" | "ORFA" | "MISTA";
type ValidacaoOrigemItem = { lancamento_id: number; competencia: string | null; descricao: string | null; origem_amigavel: string; origem_tecnica: string; referencia_item: string | null; status_origem: StatusOrigem; valor_centavos: number; motivos: string[] };
type ValidacaoOrigemFatura = { fatura_id: number; periodo_referencia: string | null; status_fatura: string | null; pode_importar_folha: boolean; possui_inconsistencia: boolean; total_fatura_centavos: number; total_validos_centavos: number; total_invalidos_centavos: number; motivos: string[]; itens: ValidacaoOrigemItem[] };
type FaturaDetalheData = { fatura: FaturaRow; conta: ContaRow | null; pessoa: PessoaRow | null; contexto_titular?: ContextoTitular | null; lancamentos: LancamentoRow[]; lancamentos_auditoria?: LancamentoRow[]; cobranca_vinculada?: CobrancaResumo | null; cobranca_canonica?: CobrancaResumo | null; pagamento_exibivel?: PagamentoExibivel | null; validacao_origem?: ValidacaoOrigemFatura | null };
...
const origemBadgeClass = (status: StatusOrigem | null | undefined) => status === "VALIDA" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : status === "CANCELADA" ? "border-amber-200 bg-amber-50 text-amber-800" : status === "ORFA" ? "border-rose-200 bg-rose-50 text-rose-700" : "border-slate-200 bg-slate-100 text-slate-700";
...
export default function FaturaDetalhePage() {
  ...
  const validacaoOrigem = data?.validacao_origem ?? null;
  const contextoTitular = data?.contexto_titular ?? null;
  ...
  const diferencaLancamentos = useMemo(
    () => Math.max(Number(fatura?.valor_total_centavos ?? 0) - somaLancamentos, 0),
    [fatura?.valor_total_centavos, somaLancamentos],
  );
  const cobrancaId = pagamento?.cobranca_exibida_id ?? ultimaCobranca?.cobranca_id ?? fatura?.cobranca_id ?? null;
  const ehColaborador = contextoTitular?.tipo === "COLABORADOR";
  const tituloConta = contextoTitular?.titular_label ?? "Conta interna";
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <Card className="border-slate-200 bg-white/95 shadow-sm"><CardContent className="space-y-6 p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">{tituloConta}</p><h1 className="mt-2 text-3xl font-semibold text-slate-950">Fatura da conta interna</h1><p className="mt-2 text-sm text-slate-600">{ehColaborador ? "Leitura operacional da conta interna do colaborador, com competência, importação em folha e vínculo financeiro." : "Leitura operacional da fatura, do pagamento NeoFin e da cobranca oficial vinculada."}</p></div>
          ...
          {loading ? <div className="text-sm text-slate-500">Carregando detalhes da fatura...</div> : error ? <div className="text-sm text-rose-700">{error}</div> : !fatura ? <div className="text-sm text-slate-500">Fatura nao encontrada.</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">ID da fatura</div><div className="mt-2 text-2xl font-semibold text-slate-950">#{fatura.id}</div><div className="mt-1 text-sm text-slate-500">{formatCompetencia(fatura.periodo_referencia)}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Titular</div><div className="mt-2 text-lg font-semibold text-slate-950">{data?.pessoa?.nome ?? "-"}</div><div className="mt-1 text-sm text-slate-500">{contextoTitular?.conta_interna_id ? `Conta interna #${contextoTitular.conta_interna_id}` : tituloConta}{data?.conta?.descricao_exibicao ? ` - ${data.conta.descricao_exibicao}` : ""}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Status</div><div className="mt-2"><span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${badgeClass(fatura.status)}`}>{fatura.status ?? "SEM STATUS"}</span></div><div className="mt-2 text-sm text-slate-500">Fechamento: {formatDate(fatura.data_fechamento)}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Valor</div><div className="mt-2 text-2xl font-semibold text-slate-950">{brl(fatura.valor_total_centavos)}</div><div className="mt-1 text-sm text-slate-500">Composicao operacional exibida: {brl(somaLancamentos)}</div>{diferencaLancamentos > 0 ? <div className="mt-1 text-xs text-slate-500">Diferenca tecnica para o total da fatura: {brl(diferencaLancamentos)}</div> : null}</div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">{ehColaborador ? "Importacao em folha" : "Tipo de pagamento exibido"}</div><div className="mt-2 text-lg font-semibold text-slate-950">{ehColaborador ? (contextoTitular?.status_importacao_folha ?? "Sem leitura") : (pagamento?.tipo_exibicao ?? "Nao informado")}</div><div className="mt-1 text-sm text-slate-500">{ehColaborador ? `Competencia: ${formatCompetencia(contextoTitular?.competencia ?? fatura.periodo_referencia)}` : `Vencimento: ${formatDate(ultimaCobranca?.vencimento_iso ?? fatura.data_vencimento)}`}</div></div></div>}
        </CardContent></Card>

        {!loading && !error && fatura ? <>
          {ehColaborador ? <Box title="Integracao com folha" subtitle="Semantica especifica da conta interna do colaborador."><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><Info label="Conta interna" value={contextoTitular?.conta_interna_id ? `#${contextoTitular.conta_interna_id}` : "-"} /><Info label="Competencia" value={formatCompetencia(contextoTitular?.competencia ?? fatura.periodo_referencia)} /><Info label="Importacao em folha" value={contextoTitular?.status_importacao_folha ?? "-"} /><Info label="Folha colaborador" value={contextoTitular?.folha_pagamento_colaborador_id ? `#${contextoTitular.folha_pagamento_colaborador_id}` : "Ainda nao vinculada"} /><Info label="Folha pagamento" value={fatura.folha_pagamento_id ? `#${fatura.folha_pagamento_id}` : "Nao vinculada"} /></div><div className="flex flex-wrap gap-2">{contextoTitular?.colaborador_id ? <Link className={SECONDARY_LINK_CLASS} href={`/financeiro/colaboradores/${contextoTitular.colaborador_id}?competencia=${contextoTitular?.competencia ?? ""}`}>Abrir ficha financeira</Link> : null}{contextoTitular?.colaborador_id ? <Link className={SECONDARY_LINK_CLASS} href={`/financeiro/colaboradores/${contextoTitular.colaborador_id}/conta-interna`}>Abrir conta interna individual</Link> : null}{contextoTitular?.folha_pagamento_colaborador_id ? <Link className={SECONDARY_LINK_CLASS} href={`/admin/financeiro/folha/colaboradores/${contextoTitular.folha_pagamento_colaborador_id}`}>Abrir folha da competencia</Link> : null}</div></Box> : null}
          {validacaoOrigem ? <Box title="Rastreabilidade da origem" subtitle="Validacao da cadeia matricula -> cobranca -> lancamento -> fatura."><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><Info label="Competencia" value={formatCompetencia(validacaoOrigem.periodo_referencia ?? fatura.periodo_referencia)} /><Info label="Status da fatura" value={validacaoOrigem.status_fatura ?? "-"} /><Info label="Importavel na folha" value={validacaoOrigem.pode_importar_folha ? "Sim" : "Nao"} /><Info label="Total valido" value={brl(validacaoOrigem.total_validos_centavos)} /><Info label="Total invalido" value={brl(validacaoOrigem.total_invalidos_centavos)} /></div><div className="mt-1 flex flex-wrap gap-2">{Array.from(new Set(validacaoOrigem.itens.map((item) => item.status_origem))).map((status) => <span key={status} className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${origemBadgeClass(status)}`}>{status}</span>)}</div>{validacaoOrigem.motivos.length > 0 ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">{validacaoOrigem.motivos.join(" | ")}</div> : <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">Nenhuma inconsistencia detectada na origem desta fatura.</div>}<div className="space-y-3">{validacaoOrigem.itens.map((item) => <div key={item.lancamento_id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-base font-semibold text-slate-950">Lancamento #{item.lancamento_id}</div><div className="mt-1 text-sm text-slate-600">{item.descricao ?? item.origem_amigavel}</div></div><div className="text-right"><span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${origemBadgeClass(item.status_origem)}`}>{item.status_origem}</span><div className="mt-2 text-sm font-semibold text-slate-950">{brl(item.valor_centavos)}</div></div></div><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Info label="Origem amigavel" value={item.origem_amigavel} /><Info label="Competencia" value={formatCompetencia(item.competencia)} /><Info label="Referencia" value={item.referencia_item ?? "-"} /><Info label="Descricao" value={item.descricao ?? "-"} /></div><div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700">{item.origem_tecnica}</div>{item.motivos.length > 0 ? <div className="mt-3 text-xs text-amber-700">{item.motivos.join(" | ")}</div> : null}</div>)}</div></Box> : null}
          ...
          <Box title="Lancamentos vinculados a fatura" subtitle="Leitura humana da composicao operacional ativa, sem itens cancelados.">
            {lancamentos.length === 0 ? <div className="text-sm text-slate-500">Nenhum lancamento operacional ativo vinculado a esta fatura.</div> : <div className="space-y-4">{lancamentos.map((l) => { const itens = Array.isArray(l.itens_operacionais) ? l.itens_operacionais : []; return <div key={l.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-base font-semibold text-slate-950">{l.descricao_operacional ?? l.descricao ?? "Lancamento"}</div><div className="mt-1 text-sm text-slate-600">{formatCompetencia(l.competencia)} | {l.aluno_nome ?? "Aluno nao identificado"} | {l.responsavel_financeiro_nome ?? "Responsavel nao identificado"}</div><div className="text-xs text-slate-500">ID #{l.id} | Origem: {l.origem_sistema ?? "-"}</div></div><div className="text-right"><div className="text-lg font-semibold text-slate-950">{brl(l.valor_centavos)}</div><div className="text-xs text-slate-500">Ref. {l.cobranca_fatura_id ? `cobranca #${l.cobranca_fatura_id}` : l.referencia_item ?? "-"}</div></div></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Info label="Fonte" value={typeof l.composicao_json?.fonte === "string" ? l.composicao_json.fonte : "-"} /><Info label="Competencia" value={formatCompetencia(l.competencia)} /><Info label="Aluno" value={l.aluno_nome ?? "-"} /><Info label="Responsavel" value={l.responsavel_financeiro_nome ?? "-"} /></div>{itens.length > 0 ? <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Itens consumidos / comprados</div><div className="mt-3 space-y-3">{itens.map((item, index) => <div key={`${l.id}-${index}`} className="flex items-start justify-between gap-4 rounded-xl border border-slate-100 bg-slate-50 px-3 py-3"><div><div className="font-medium text-slate-950">{item.descricao}</div><div className="text-xs text-slate-500">{`${item.quantidade ?? 1}x`}{item.valor_unitario_centavos !== null ? ` • ${brl(item.valor_unitario_centavos)} un.` : ""}{item.detalhe ? ` • ${item.detalhe}` : ""}</div></div><div className="text-sm font-semibold text-slate-950">{brl(item.valor_total_centavos)}</div></div>)}</div></div> : null}<details className="mt-4 rounded-2xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-medium text-slate-900">Ver JSON tecnico</summary><pre className="mt-3 max-h-80 overflow-auto rounded-xl bg-slate-50 p-3 text-xs text-slate-700">{JSON.stringify(l.composicao_json ?? {}, null, 2)}</pre></details></div>; })}</div>}
          </Box>
