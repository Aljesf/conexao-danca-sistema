      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <Card className="border-slate-200 bg-white/95 shadow-sm"><CardContent className="space-y-6 p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">{podeFechar && !invoiceValida ? <Button onClick={() => { setVencimentoFechar(defaultVencimento(fatura?.data_vencimento)); setModalFecharOpen(true); }}>Fechar fatura</Button> : null}{!invoiceValida ? <Button variant="secondary" onClick={() => { setVencimentoGerar(defaultVencimento(fatura?.data_vencimento)); setModalCobrancaOpen(true); }}>Gerar cobranca agora</Button> : null}{pagamento?.link_pagamento ? <a href={pagamento.link_pagamento} target="_blank" rel="noreferrer" className={SECONDARY_LINK_CLASS}>Abrir segunda via</a> : null}{cobrancaId ? <Button variant="secondary" onClick={() => void sincronizarNeofin()} disabled={sincronizandoPagamento}>{sincronizandoPagamento ? "Sincronizando..." : "Sincronizar NeoFin"}</Button> : null}<Button variant="secondary" onClick={() => router.back()}>Voltar</Button><Button variant="secondary" onClick={() => void carregarFatura()}>Recarregar</Button></div>
          </div>
        ...
        {!loading && !error && fatura ? <>
          <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <Box title="Pagamento" subtitle="Dados resolvidos da invoice NeoFin ou do payload local.">
              <div className="grid gap-4 md:grid-cols-2"><Info label="Tipo exibicao" value={pagamento?.tipo_exibicao ?? "-"} /><Info label="Status sincronizado" value={<span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${badgeClass(pagamento?.status_sincronizado)}`}>{pagamento?.status_sincronizado ?? "-"}</span>} /><Info label="Invoice NeoFin" value={pagamento?.invoice_id ?? "-"} /><Info label="Charge resolvido" value={pagamento?.neofin_charge_id ?? "-"} /><Info label="Origem dos dados" value={paymentOrigin(pagamento?.origem_dos_dados)} /><Info label="Segunda via disponivel" value={pagamento?.segunda_via_disponivel ? "Sim" : "Nao"} /></div>
              <div className="flex flex-wrap gap-2">{copyable(pagamento?.linha_digitavel) ? <Button variant="secondary" onClick={() => void copy(pagamento?.linha_digitavel as string, "Linha digitavel copiada.", setToast)}>Copiar linha digitavel</Button> : null}{copyable(pagamento?.pix_copia_cola) ? <Button variant="secondary" onClick={() => void copy(pagamento?.pix_copia_cola as string, "Pix copia e cola copiado.", setToast)}>Copiar Pix</Button> : null}{pagamento?.qr_code_url ? <a href={pagamento.qr_code_url} target="_blank" rel="noreferrer" className={SECONDARY_LINK_CLASS}>Abrir QR Pix</a> : null}</div>
              <div className="grid gap-3"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Link de pagamento</div><div className="mt-2 break-all text-sm text-slate-900">{pagamento?.link_pagamento ?? "-"}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Linha digitavel</div><div className="mt-2 break-all font-mono text-sm text-slate-900">{pagamento?.linha_digitavel ?? "-"}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Codigo de barras</div><div className="mt-2 break-all font-mono text-sm text-slate-900">{pagamento?.codigo_barras ?? "-"}</div></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Pix copia e cola</div><div className="mt-2 break-all font-mono text-sm text-slate-900">{pagamento?.pix_copia_cola ?? "-"}</div></div>{pagamento?.qr_code_url ? <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">QR Pix</div><div className="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-white p-3"><Image src={pagamento.qr_code_url} alt="QR Pix da fatura" width={320} height={320} unoptimized loader={() => pagamento.qr_code_url ?? ""} className="mx-auto max-h-72 w-full max-w-xs object-contain" /></div></div> : null}</div>
