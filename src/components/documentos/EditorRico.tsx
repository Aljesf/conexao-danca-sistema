...
export function EditorRico({ valueHtml, onChangeHtml, variaveis }: Props) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-[240px] w-full rounded-md border border-slate-200 bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-slate-300",
      },
    },
  });
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-slate-500">
        Placeholders entram como <code>{"{{CODIGO}}"}</code>. Ex.: <code>{"{{ALUNO_NOME}}"}</code>.
      </p>
    </div>
  );
}
