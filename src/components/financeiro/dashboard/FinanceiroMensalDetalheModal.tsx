...
export function FinanceiroMensalDetalheModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl overflow-hidden rounded-[28px] border border-slate-200/80 bg-white p-0 shadow-[0_28px_80px_-42px_rgba(15,23,42,0.45)]">
        <div className="border-b border-slate-200/80 px-6 py-5">
          <DialogHeader>
            <DialogTitle>{payload?.titulo ?? "Detalhamento"}</DialogTitle>
            {payload?.subtitulo ? (
              <DialogDescription>{payload.subtitulo}</DialogDescription>
            ) : null}
          </DialogHeader>
